...
// --- Bun Advanced String & Signature Analysis ---
// Demonstrates advanced reverse engineering utilities inside the Bun runtime.
function runDemo() {
  console.log("Constructing mock x64 binary in memory (Bun)...");
  ...
  // - String "Hello_Bun\0" starts at offset 20.
  const mockBinary = Buffer.concat([
    ...
    Buffer.from("Hello_Bun\0", "utf8")     // 20-29: Null-terminated string literal
  ]);

  // Pass to BinaryAnalyzer
  const analyzer = new BinaryAnalyzer(mockBinary);

  // 1. Find string offset
  const strOffset = analyzer.findString("Hello_Bun");
  console.log(`1. String "Hello_Bun" found at offset: 0x${strOffset?.toString(16)} (index ${strOffset})`);
  ...
  if (strOffset !== null) {
    ...
    const sigResult = analyzer.generateSignatureFromString("Hello_Bun", 11);
    if (sigResult) {
